...
| staging 回收 | `teardown-staging.ps1` 可清理容器与卷 | △ | 已补 `-DryRun` 非破坏演练并验证命令预览，见 `docs/dev/Phase13-staging-teardown-dryrun-20260427.md`；实际停容器/删卷仍需独立回收窗口 |
| 指标 | `/metrics` 可抓取 | ✅ | 已用于 TD-04 补采样，Prometheus 文本格式可直接 `curl` 抓取 |
| 手册 | 非作者按文档独立完成一次拉起与回收 | △ | 已按手册复现非破坏回收演练；完整拉起+实际回收仍需独立窗口 |
