...
- 网关验证签名与有效期；JWT Claims 见 [G1 §5.3](数据模型与权限设计.md)

### 3.2 鉴权粒度

- **湖泊级：** 网关查询 PostgreSQL `lake_memberships`，判断用户是否属于 `lake_id`
- **节点级：** 转发到后端，后端按 [G1 §五权限矩阵](数据模型与权限设计.md) 校验
...
| AI 调用 | 见 [G5 §六 Token 预算](AI服务编排-织网工作流.md) |
...
- 鉴权数据：[G1 §五](数据模型与权限设计.md)
- WebSocket 协作：[D5](多人实时协作设计.md)
