...
// ─────────────────────────────────────────────────────────────────────────────
// GraphScene — 仅在 Canvas 内渲染，节点数/边数已裁剪到上限
// ─────────────────────────────────────────────────────────────────────────────
...
function GraphScene({ displayNodes, displayEdges, onNodeSelect }: SceneProps) {
  ...
  const { positions, lineObj } = useMemo(() => {
    ...
    for (const n of simNodes) positions.set(n.id, [n.x ?? 0, n.y ?? 0])
      ...
      {displayNodes.map(node => {
        ...
        return (
          <mesh
            key={node.id}
            position={[x, y, 0]}
            onClick={e => handleNodeClick(node, () => e.stopPropagation())}
          >
            <primitive object={sel ? SPHERE_GEO_SELECTED : SPHERE_GEO_NORMAL} attach="geometry" />
            <meshStandardMaterial
              color={color}
              emissive={sel ? color : '#000000'}
              emissiveIntensity={sel ? 0.5 : 0}
            />
            {sel && (
              <Html
                position={[0, 12, 0]}
